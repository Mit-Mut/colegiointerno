...
class AccountInvoiceLine(models.Model):
    _inherit = 'account.move.line'  # Se cambio 'account.invoice.line'

    @api.onchange('product_id')
    def _onchange_product_id(self):
        res = super()._onchange_product_id()
        #Se cambio invoice_id y type
        ...
        if self.product_id and inv_type:
            ...
            # Se modifico account_analytic_id
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if vals.get('product_id') and inv_type and \
                    not vals.get('analytic_account_id'):    # Se modifico account_analytic_id
                product = self.env['product.product'].browse(
                    vals.get('product_id'))
                ana_accounts = product.product_tmpl_id.\
                    _get_product_analytic_accounts()
                ana_account = ana_accounts[INV_TYPE_MAP[inv_type]]
                vals['analytic_account_id'] = ana_account.id    # Se modifico account_analytic_id
